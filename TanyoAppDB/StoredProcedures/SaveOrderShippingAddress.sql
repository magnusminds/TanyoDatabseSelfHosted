CREATE PROCEDURE [dbo].[SaveOrderShippingAddress] (
	@OrderId BIGINT
...
BEGIN
	...
	BEGIN TRY
		...
		BEGIN
			...
			WHERE ca.CustomerAddressId = @ShippingAddressID
				AND ca.IsDeleted = 0

			SELECT CAST(1 AS BIT) AS [Status]
				,'Shipping address inserted successfully.' AS [Message]
		END
		ELSE
		BEGIN
			-- Update if exists
			...
			INNER JOIN dbo.CustomerAddresses ca WITH (NOLOCK) ON ca.CustomerAddressId = @ShippingAddressID
			INNER JOIN dbo.Customers c WITH (NOLOCK) ON c.CustomerId = ca.CustomerId
			LEFT JOIN dbo.CustomerAddresses def_ca WITH (NOLOCK)
				ON def_ca.CustomerId = c.CustomerId
				AND def_ca.IsDefault = 1
			...
			SELECT CAST(1 AS BIT) AS [Status]
				,'Shipping address updated successfully.' AS [Message]
		END
		...
		BEGIN
			EXEC dbo.SaveActivityLog
				...
				,@SubjectId = @OrderId
				,@Description = N'Shipping address has been changed.'
				,@Action = 'UPDATE'
				,@CreatedBy = @UserId
