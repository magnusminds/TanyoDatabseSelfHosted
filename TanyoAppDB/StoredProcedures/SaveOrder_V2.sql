CREATE PROCEDURE [dbo].[SaveOrder_V2] (
	@OrderID BIGINT
	,@OrderType VARCHAR(1)
	,@TenantID BIGINT
	,@UserID BIGINT
...
BEGIN
	...
	BEGIN TRY
		...
		IF OBJECT_ID('tempdb..#tmpOrderSetItemAttachments') IS NOT NULL
			DROP TABLE #tmpOrderSetItemAttachments;

		CREATE TABLE #tmpOrderSetItemAttachments (
			ID BIGINT IDENTITY
			,OrderSetItemGroupID BIGINT
			,GroupID BIGINT
			,IsImage BIT
		...
		CROSS APPLY OPENJSON(OrderSetItems, '$') WITH (
				...
				,UnitSalePrice NVARCHAR(MAX) '$.UnitSalePrice'
		...
		INSERT INTO #tmpOrderSetItemAttachments (
			OrderSetItemGroupID
			,GroupID
			,IsImage
			,FileUrl
			)
		SELECT osi.GroupID AS OrderSetItemGroupID
			,att.GroupID
			,att.IsImage
			,att.FileUrl
		FROM OPENJSON(@JsonObject, '$.OrderSetRequestDto') WITH (OrderSetItems NVARCHAR(MAX) '$.OrderSetItemRequestDto' AS JSON) oss
		CROSS APPLY OPENJSON(OrderSetItems) WITH (
				GroupID BIGINT '$.groupId'
				,Attachments NVARCHAR(MAX) '$.Attachments' AS JSON
				) osi
		CROSS APPLY OPENJSON(osi.Attachments) WITH (
				GroupID BIGINT '$.groupId'
				,IsImage BIT '$.IsImage'
				,FileUrl VARCHAR(MAX) '$.FileURL'
				) att;

		UPDATE tosi
		SET tosi.InstantCostPrice = CASE 
				WHEN c.IsSellByPerSQFT = 1
					THEN [dbo].[GetProductPriceBySQFT](@RoleId, @TenantID, p.ProductId, 1)
				WHEN (
						p.Width = tosi.Width
				...
				END
			,tosi.IsSellByPerSQFT = c.IsSellByPerSQFT
		FROM #tmpOrderSetItems tosi
		INNER JOIN Products p WITH (NOLOCK) ON p.ProductId = tosi.SubjectId
				...
				BEGIN
					...
					BEGIN
						...
						SET osi.SubjectTypeId = ossi.SubjectTypeId
							...
							,OSI.UnitSalePrice = ossi.UnitSalePrice
							,OSI.MRP = CASE 
								WHEN ossi.OfferId > 0
									AND ossi.IsSellByPerSQFT = 1
									THEN [dbo].[GetProductPriceBySQFT](@RoleId, @TenantID, ossi.SubjectId, 2)
								WHEN ossi.OfferId > 0
									AND ossi.IsSellByPerSQFT = 0
									THEN [dbo].[GetProductPriceByDimension](@RoleId, @TenantId, ossi.SubjectId, ossi.Width, ossi.Height, ossi.Depth, 2)
								ELSE 0
								END
					...
					BEGIN
						...
						SELECT @NewOrderID
							...
							,CASE 
								WHEN osi.OfferId > 0
									AND osi.IsSellByPerSQFT = 1
									THEN [dbo].[GetProductPriceBySQFT](@RoleId, @TenantID, osi.SubjectId, 2)
								WHEN osi.OfferId > 0
									AND osi.IsSellByPerSQFT = 0
						...
						LEFT JOIN Offers OFR ON OFR.OfferId = osi.OfferId
					...
					IF EXISTS (
							SELECT 1
							FROM #tmpOrderSetItemAttachments a
							INNER JOIN #tmpOrderSetItems i ON i.GroupID = a.OrderSetItemGroupID
							WHERE i.ID = @incSetItems
							)
					BEGIN
						INSERT INTO OrderSetItemImages (
							OrderSetItemId
							,DrawImage
							,CreatedBy
							,CreatedDate
					...
					BEGIN
						...
						BEGIN
							...
							SET osi.SubjectTypeId = osia.SubjectTypeId
								...
								,OSI.UnitSalePrice = osia.UnitSalePrice
								,OSI.MRP = CASE 
									WHEN osia.OfferId > 0
										AND osia.IsSellByPerSQFT = 1
										THEN [dbo].[GetProductPriceBySQFT](@RoleId, @TenantID, osia.SubjectId, 2)
									WHEN osia.OfferId > 0
										AND osia.IsSellByPerSQFT = 0
						...
						BEGIN
							...
							LEFT JOIN Offers OFR ON OFR.OfferId = osia.OfferId
							WHERE OrderSetItemGroupID = @OrderSetItemGroupId
								AND OrderSetGroupID = @OrderSetGroupID
								AND GroupID = @incSetItemAddOns
		...
		--Capture Order Request JSON for analysis
		INSERT INTO dbo.OrderRequestLog (
			TenantId
			,OrderId
			,OrderNo
			,UserId
	...
	BEGIN CATCH
		...
		EXEC dbo.SaveDBErrorLog 
			...
			,@ErrorMsg = @ErrorMsg;
			--SET @ErrorStatus =1
			--SELECT  @ErrorStatus AS ReturnStatus
			--,@ErrorMsg AS ReturnMessage
	END CATCH
END
