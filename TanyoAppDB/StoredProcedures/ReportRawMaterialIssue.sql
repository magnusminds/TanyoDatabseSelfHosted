...
BEGIN
    ...
    SELECT
        ...
        SUM(
            CASE
                WHEN ISNULL(MWD.IsNotNeeded, 0) = 1 THEN 0
                WHEN MWD.RequiredQty IS NULL THEN 0
    ...
    INNER JOIN RawMaterials RM WITH (NOLOCK)
        ON MWD.RawMaterialId = RM.RawMaterialId
        AND RM.TenantId = @TenantId
    INNER JOIN ManufacturingWorkOrders MWO WITH (NOLOCK)
        ON MWO.ManufacturingWorkOrderId = MWD.ManufacturingWorkOrderId
    ...
    WHERE
        ...
        AND
        (
            (@IssuedFromDate IS NULL AND @IssuedToDate IS NULL)
            OR EXISTS
            (
                SELECT 1
                ...
                WHERE MWML.ManufacturingWorkOrderDetailId = MWD.ManufacturingWorkOrderDetailId
                    ...
                    AND (@IssuedFromDate IS NULL OR CAST(MWML.CreatedDate AS DATE) >= @IssuedFromDate)
                    AND (@IssuedToDate IS NULL OR CAST(MWML.CreatedDate AS DATE) <= @IssuedToDate)
            )
        )
    ...
    SELECT
        ...
        (
            ...
            INNER JOIN ManufacturingWorkOrders MWO_I WITH (NOLOCK)
                ON MWO_I.ManufacturingWorkOrderId = MWOD_I.ManufacturingWorkOrderId
            INNER JOIN Orders ORD_I WITH (NOLOCK)
                ON ORD_I.OrderId = MWO_I.OrderId
            INNER JOIN ManufacturingWorkOrderDetailsLog MWML WITH (NOLOCK)
                ON MWML.ManufacturingWorkOrderDetailId = MWOD_I.ManufacturingWorkOrderDetailId
                AND MWML.RawMaterialId = MWOD_I.RawMaterialId
            WHERE
                ISNULL(MWO_I.IsDeleted, 0) = 0
    ...
    INNER JOIN RawMaterials RM WITH (NOLOCK)
        ON RMI.RawMaterialId = RM.RawMaterialId
        AND RM.TenantId = @TenantId
    INNER JOIN #RawMaterials RMS
        ON RM.RawMaterialId = RMS.RawMaterialId
    WHERE
        (
            @RawMaterialName IS NULL
            OR RM.Title LIKE '%' + @RawMaterialName + '%'
        ...
        (
            ...
            (
                ...
                INNER JOIN ManufacturingWorkOrders MWO2 WITH (NOLOCK)
                    ON MWO2.ManufacturingWorkOrderId = MWOD2.ManufacturingWorkOrderId
    ...
    ORDER BY
        CASE WHEN @SortBy = 'ProvidedQty' AND @SortOrder = 'ASC' THEN RMS.ProvidedQty END ASC,
        CASE WHEN @SortBy = 'ProvidedQty' AND @SortOrder = 'DESC' THEN RMS.ProvidedQty END DESC,
        ...
        CASE WHEN @SortBy = 'RequiredQty' AND @SortOrder = 'ASC' THEN RMS.RequiredQty END ASC,
        CASE WHEN @SortBy = 'RequiredQty' AND @SortOrder = 'DESC' THEN RMS.RequiredQty END DESC,
        CASE WHEN @SortBy = 'Inventory' AND @SortOrder = 'ASC' THEN RMI.Inventory END ASC,
        CASE WHEN @SortBy = 'Inventory' AND @SortOrder = 'DESC' THEN RMI.Inventory END DESC,
        CASE WHEN @SortBy = 'IssuedDate' AND @SortOrder = 'ASC' THEN
            (
                SELECT MAX(MWML.CreatedDate)
                FROM ManufacturingWorkOrderDetails MWOD_S WITH (NOLOCK)
                INNER JOIN ManufacturingWorkOrders MWO_S WITH (NOLOCK)
                    ON MWO_S.ManufacturingWorkOrderId = MWOD_S.ManufacturingWorkOrderId
                INNER JOIN ManufacturingWorkOrderDetailsLog MWML WITH (NOLOCK)
                    ON MWML.ManufacturingWorkOrderDetailId = MWOD_S.ManufacturingWorkOrderDetailId
                    AND MWML.RawMaterialId = MWOD_S.RawMaterialId
                WHERE MWOD_S.RawMaterialId = RM.RawMaterialId
                    AND ISNULL(MWO_S.IsDeleted, 0) = 0
            )
        END ASC,
        CASE WHEN @SortBy = 'IssuedDate' AND @SortOrder = 'DESC' THEN
            (
                SELECT MAX(MWML.CreatedDate)
                FROM ManufacturingWorkOrderDetails MWOD_S WITH (NOLOCK)
                INNER JOIN ManufacturingWorkOrders MWO_S WITH (NOLOCK)
