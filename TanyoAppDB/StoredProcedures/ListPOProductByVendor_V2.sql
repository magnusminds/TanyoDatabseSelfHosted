/*
SELECT * FROM Vendors WHERE TenantId = 2
	EXEC [dbo].[ListPOProductByVendor_V2]
		 @TenantId = 2
    ,@VendorId = 4
    ,@Search = NULL
*/
CREATE   PROCEDURE [dbo].[ListPOProductByVendor_V2]
(
     @TenantId BIGINT 
    ,@VendorId BIGINT 
...
BEGIN
	...
	SELECT p.ProductId
		...
		,CAST(ISNULL(v.GSTType, 1) AS BIT) GSTType
		...
		,CAST(CASE WHEN pvm.ProductId IS NOT NULL THEN 1 ELSE 0 END AS BIT) AS IsMapped --1 IsMapped with Vendor, 2 Not mapped with Product
	...
	LEFT JOIN ProductVendorMapping pvm WITH (NOLOCK)
            ...
            AND pvm.IsDeleted  = 0
	LEFT JOIN Vendors v WITH (NOLOCK) ON v.VendorId = pvm.VendorId
