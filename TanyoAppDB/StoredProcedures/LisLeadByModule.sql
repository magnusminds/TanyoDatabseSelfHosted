...
BEGIN
    ...
    BEGIN TRY
        ...
        SELECT 
            ...
            ,ISNULL(NULLIF(CONCAT(cust.FirstName,' ',cust.LastName),''),
        ...
        LEFT JOIN LookupValues LKV WITH (NOLOCK)
            ON l.LeadSourceId = LKV.LookupValueId
        LEFT JOIN Labels lbl WITH (NOLOCK)
            ON l.Priority = lbl.LabelId
        ...
        LEFT JOIN Customers custRef WITH (NOLOCK)
            ON cust.RefferedBy = custRef.CustomerId
            AND custRef.TenantId = @TenantId
        LEFT JOIN LookupValues BUY WITH (NOLOCK) 
            ON l.BuyingRangeValueId = BUY.LookupValueId
        OUTER APPLY (
            SELECT TOP 1 FollowUpDate, FollowUpComment
        ...
        ORDER BY 
            CASE WHEN @SortBy = 'Priority' AND @SortOrder = 'ASC' THEN l.Priority END ASC,
            CASE WHEN @SortBy = 'Priority' AND @SortOrder = 'DESC' THEN l.Priority END DESC,

            CASE WHEN @SortBy = 'InquiryDate' AND @SortOrder = 'ASC' THEN l.CreatedDate END ASC,
            CASE WHEN @SortBy = 'InquiryDate' AND @SortOrder = 'DESC' THEN l.CreatedDate END DESC,

            CASE WHEN @SortBy = 'Status' AND @SortOrder = 'ASC' THEN l.Status END ASC,
            CASE WHEN @SortBy = 'Status' AND @SortOrder = 'DESC' THEN l.Status END DESC,

            CASE WHEN @SortBy = 'CustomerName' AND @SortOrder = 'ASC' THEN ISNULL(NULLIF(CONCAT(cust.FirstName,' ',cust.LastName),''),
                                                                                CONCAT(l.FirstName,' ',l.LastName)) END ASC,
            CASE WHEN @SortBy = 'CustomerName' AND @SortOrder = 'DESC' THEN ISNULL(NULLIF(CONCAT(cust.FirstName,' ',cust.LastName),''),
                                                                                CONCAT(l.FirstName,' ',l.LastName)) END DESC,

            CASE WHEN @SortBy = 'PhoneNumber' AND @SortOrder = 'ASC' THEN ISNULL(cust.PhoneNumber, l.PhoneNumber) END ASC,
            CASE WHEN @SortBy = 'PhoneNumber' AND @SortOrder = 'DESC' THEN ISNULL(cust.PhoneNumber, l.PhoneNumber) END DESC,

            CASE WHEN @SortBy = 'Salesman' AND @SortOrder = 'ASC' THEN u.FirstName + ' ' + u.LastName END ASC,
            CASE WHEN @SortBy = 'Salesman' AND @SortOrder = 'DESC' THEN u.FirstName + ' ' + u.LastName END DESC,

            CASE WHEN @SortBy = 'FollowUpDate' AND @SortOrder = 'ASC' THEN fu.FollowUpDate END ASC,
            CASE WHEN @SortBy = 'FollowUpDate' AND @SortOrder = 'DESC' THEN fu.FollowUpDate END DESC,

            CASE WHEN @SortBy = 'LeadSourceName' AND @SortOrder = 'ASC' THEN ISNULL(LKV.LookupValueName, ISNULL(l.Other,'Other')) END ASC,
            CASE WHEN @SortBy = 'LeadSourceName' AND @SortOrder = 'DESC' THEN ISNULL(LKV.LookupValueName, ISNULL(l.Other,'Other')) END DESC,

            CASE WHEN @SortBy = 'LeadNumber' AND @SortOrder = 'ASC' THEN l.LeadNumber END ASC,
            CASE WHEN @SortBy = 'LeadNumber' AND @SortOrder = 'DESC' THEN l.LeadNumber END DESC,

            ------------------------------------------
            -- DEFAULT SORT (FIXED)
            ------------------------------------------
            CASE WHEN @SortBy = 'UpdatedDate' AND @SortOrder = 'ASC' THEN ISNULL(l.UpdatedDate, l.CreatedDate) END ASC,
            CASE WHEN @SortBy = 'UpdatedDate' AND @SortOrder = 'DESC' THEN ISNULL(l.UpdatedDate, l.CreatedDate) END DESC,
