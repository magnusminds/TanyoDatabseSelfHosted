...
BEGIN
    ...
    SELECT
         @CustomerId   = JSON_VALUE(@ComplainRequestDetails, '$.customerDetails.customerId')
    ...
    SELECT
         @CustomerAddressId = JSON_VALUE(@ComplainRequestDetails, '$.addressDetails.customerAddressId')
        ...
        ,@IsDefault         = CAST(ISNULL(JSON_VALUE(@ComplainRequestDetails, '$.addressDetails.isDefault'), '0') AS BIT)
    ...
    SELECT
         @ComplainId     = JSON_VALUE(@ComplainRequestDetails, '$.complainDetails.complainId')
        ,@OrderId        = JSON_VALUE(@ComplainRequestDetails, '$.complainDetails.orderId')
    ...
    IF @Status IS NULL
    BEGIN
        SET @ReturnMessage = 'Complain status is required.';
    ...
    BEGIN
        SET @ReturnMessage = 'Complain not found.';
        THROW 50001, @ReturnMessage, 1;
    END

    BEGIN TRY

    BEGIN TRANSACTION UpdateComplain;
        ...
        SET
            ...
            ,CustomerAddressId = CASE 
                                    WHEN @SkipAddress = 0 THEN @CustomerAddressId 
                                    ELSE CustomerAddressId 
                                END
            ,Address         = CASE
                                   WHEN @SkipAddress = 0
                                   THEN CONCAT_WS(', ',
        ...
        COMMIT TRANSACTION UpdateComplain;
    ...
    BEGIN CATCH
        IF @@TRANCOUNT > 0
            ROLLBACK TRANSACTION UpdateComplain;
