CREATE     PROC [dbo].[DeletePORawMaterials]
(
...
BEGIN
	...
	BEGIN TRY
		...
		IF EXISTS (
			SELECT 1
			FROM dbo.PORawMaterials WITH (NOLOCK)
	...
	BEGIN CATCH
		...
		RAISERROR('DeletePORawMaterials failed: %s (Line %d)', @ErrorSeverity, @ErrorState, @ErrorMsg, @ErrorLine)
	END CATCH
END
